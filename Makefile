#
# Copyright 2018 Adam Greene (See LICENSE)
#
# Universal tool for deploying packages or testing packages on Python2
# or Python3. Includes pip, setuptools, etc. for systems that don't
# already have any system or user packages installed, e.g. old UNIX
# systems or in Docker containers, or whatever. Just use either:
#
# * `make python2`
# * `make python3`
#
...
#
# This Makefile also includes a very useful target that can be used for
# tag-based release mgmt with versioneer, using `make release`. This
# will automatically bump and push a tag as well as publish a the package
# to a specific PyPi compatible repo (Artifactory, etc) if .pypirc is
# configured properly (see `make pypirc`)
#
# MacOS works, with some tweaks (use `make PYTHON=/path/to/system/python`)
# Supported Targets:
...
OVERWATCH_DIR = overwatch-exports/$(DATE)
...
PyBuild23 - https://github.com/mzpqnxow/pybuild23
    ...
    You probably want `make python3`
...
python2: $(VENV_DIR) clean
	@echo Python2 is no longer supported
	@/bin/false
...
doc:
	...
	pandoc  LICENSE.md -o LICENSE.pdf "-fmarkdown-implicit_figures -o" --from=markdown -V geometry:margin=.4in --toc --highlight-style=espresso
